...
"""
                ...
                if st.session_state.get("edit_mode"):
                    with st.form("edit"):
                        ...
                        with e2:
                            ...
                            ebo = ["기본형","연계형","기타"]
                            cub = v("구축형") if v("구축형") in ebo else "기본형"
                            ebuild = st.selectbox("구축형", ebo, index=ebo.index(cub))
                        render_section_title("고객사 담당자")
                        ec1, ec2, ec3 = st.columns(3)
                        with ec1: ecn = st.text_input("담당자명", value=str(v("고객담당자")))
                        with ec2:
                            to = ["인사팀","재무팀","총무팀","IT/전산팀","기타"]
                            ct = v("담당부서") if v("담당부서") in to else "기타"
                            ect = st.selectbox("부서", to, index=to.index(ct))
                        with ec3: ecp = st.text_input("연락처", value=str(v("담당연락처")))
                        render_section_title("관리 정보")
                        em1, em2 = st.columns(2)
                        with em1:
                            mo = ["전준수","임인지","이수현","길민종","맹국성","이성환","기타"]
                            cm = v("담당자") if v("담당자") in mo else "기타"
                            emgr = st.selectbox("영업 담당자", mo, index=mo.index(cm))
                            mto = ["일반관리","중점관리","VIP","해지예상"]
                            cmt = v("관리구분") if v("관리구분") in mto else "일반관리"
                            emtype = st.selectbox("관리구분", mto, index=mto.index(cmt))
                            eimpl = st.text_input("개설/이행일", value=str(v("개설이행일")))
                        with em2:
                            oo = ["개설완료","개설대기"]
                            co = v("개설구분") if v("개설구분") in oo else "개설대기"
                            eopen = st.selectbox("개설구분", oo, index=oo.index(co))
                            edate = st.text_input("신규접수일", value=str(v("신규접수일")))
                            bo = ["신규","재계약"]
                            cb2 = v("구축구분") if v("구축구분") in bo else "신규"
                            ebg = st.selectbox("구축구분", bo, index=bo.index(cb2))
                            ecode = st.text_input("관리코드", value=str(v("관리코드")))
                            lo = ["ERP연계대기","ERP연계진행","ERP연계취소","ERP연계완료","ERP 청구완료","연계청구보류"]
                            cl2 = v("연계상태") if v("연계상태") in lo else "ERP연계대기"
                            elink = st.selectbox("연계상태", lo, index=lo.index(cl2))
                        if st.form_submit_button("수정 저장", type="primary"):
                            up = {
                                "고객번호": c_no, "사업자번호": normalize_digits(ebiz),
                                "고객명": ename, "구축형": ebuild, "고객담당자": ecn,
                                "담당부서": ect, "담당연락처": ecp, "담당자": emgr,
                                "관리구분": emtype, "개설구분": eopen, "신규접수일": edate,
                                "구축구분": ebg, "관리코드": ecode, "개설이행일": eimpl,
                                "연계상태": elink
                            }
                ...
                if memos:
                    ...
                    for m in reversed(memos):
                        mih += f'<div style="background:#fff;padding:14px 18px;margin-bottom:8px;border-radius:10px;border:1px solid #dfe3e8;border-left:3px solid #008485;"><div style="display:flex;align-items:center;gap:10px;margin-bottom:8px;padding-bottom:8px;border-bottom:1px solid #eef1f5;"><span style="font-weight:700;font-size:12px;color:#006a6b;background:#e0f2f2;padding:2px 10px;border-radius:20px;">{m.get("Writer","")}</span><span style="font-size:11px;color:#8c95a6;">{m.get("Date","")}</span></div><div style="font-size:14px;color:#1a1a2e;line-height:1.7;">{m.get("Memo","")}</div></div>'
            ...
            if st.form_submit_button("신규 고객 저장하기", type="primary", use_container_width=True):
                ...
                if not ok:
                    ...
                    if not dok:
                        ...
                        d = {
                            "고객번호": str(i1).strip(), "사업자번호": normalize_digits(i2),
                            "고객명": str(i3).strip(), "구축형": str(i4), "담당자": str(i5),
                            "관리구분": str(i6), "구축구분": str(i7), "관리코드": str(i8),
                            "개설구분": str(i0), "신규접수일": str(i9),
                            "고객담당자": str(icn).strip(), "담당부서": str(ict),
                            "담당연락처": str(icp).strip(), "개설이행일": str(iimp).strip(),
                            "연계상태": str(ilnk)
                        }
                        suc, msg2 = add_fast(d)
                        if suc:
    ...
    if tab3 is not None:
        with tab3:
            ...
            try:
                df = get_current_df()
                sd = st.text_input("삭제 검색", placeholder="고객명 등...")
                if sd:
                    df = df[df.astype(str).apply(lambda x: x.str.contains(sd, regex=False)).any(axis=1)]
